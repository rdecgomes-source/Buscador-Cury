...
export default function App() {
    ...
    try {
      ...
      tipologiasBase.forEach((t) => {
        ...
        const item = {
          ...
          vaga: t.Vaga || "",
          precoNumero,
          precoTexto: precoNumero ? "A partir de R$ " + Number(precoNumero).toLocaleString("pt-BR") : "Preço não informado",
          referencia: t.Referência || "",
          observacoes: t.Observações || "",
        };
        if (!tipologiasPorNome[nome]) tipologiasPorNome[nome] = [];
        tipologiasPorNome[nome].push(item);
      });
      ...
      const lista = empreendimentosBase
        ...
        .map((e) => {
          ...
          const tipologias = tipologiasPorNome[nomeChave] || [];
          const precos = tipologias.map((t) => Number(t.precoNumero || 0)).filter((n) => n > 0);
          ...
          return {
            ...
            precoTexto: menorPreco ? "A partir de R$ " + Number(menorPreco).toLocaleString("pt-BR") : "Preço não informado",
          };
        });
  ...
  const csvParaObjetos = (csv) => {
    const linhas = csv.trim().split(/\r?\n/).map((linha) =>
      linha.split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/).map((c) => c.replace(/^"|"$/g, "").replace(/""/g, '"').trim())
    );
    const cabecalho = linhas[0] || [];
    return linhas.slice(1).map((linha) => {
      const obj = {};
      cabecalho.forEach((coluna, i) => (obj[coluna] = linha[i] || ""));
      return obj;
    });
  };
  ...
  const buscar = () => {
    ...
    let encontrados = empreendimentos.filter((e) => {
      const texto = normalizar([
        e.nome, e.construtora, e.regiao, "zona " + e.regiao, e.bairro, e.endereco,
        e.numero, e.cep, e.enquadramento, e.mcmv,
        (e.tipologias || []).map((t) => `${t.tipologia} ${t.areaMin} ${t.areaMax} ${t.varanda} ${t.vaga} ${t.precoTexto}`).join(" "),
      ].join(" "));
      return !termo || texto.includes(termo) || texto.includes(termoSemZona);
    });

    if (precoMax) {
      encontrados = encontrados.filter((e) => Number(e.precoNumero || 0) > 0 && Number(e.precoNumero) <= precoMax);
    }

    encontrados.sort((a, b) => Number(a.precoNumero || 999999999) - Number(b.precoNumero || 999999999));
    setResultados(encontrados);
    setMensagem(encontrados.length ? `Encontramos ${encontrados.length} empreendimento(s).` : "Nenhum empreendimento encontrado.");
  };
    ...
    return (
      <div style={styles.loginPage}>
        <div style={styles.loginBox}>
          ...
          <input type="password" placeholder="Digite a senha" value={senha} onChange={(e) => setSenha(e.target.value)} onKeyDown={(e) => e.key === "Enter" && verificarSenha()} style={styles.input} />
          <button onClick={verificarSenha} style={styles.primaryButton}>Entrar</button>
        </div>
      </div>
    );
  }
      ...
      <div style={styles.container}>
        <div style={styles.header}>
          ...
          <div>
            ...
            <p style={styles.text}>Dados atualizados direto da planilha Google</p>
          </div>
        </div>

        <div style={styles.box}><span>Empreendimentos cadastrados</span><h2>{carregando ? "Carregando..." : total}</h2></div>
        {erro ? <div style={styles.error}>{erro}</div> : null}

        <div style={styles.searchBox}>
          <input placeholder="Digite CEP, bairro, região ou nome do empreendimento" value={busca} onChange={(e) => setBusca(e.target.value)} onKeyDown={(e) => e.key === "Enter" && buscar()} style={styles.input} />
          <input type="number" placeholder="Preço máximo" value={precoMaximo} onChange={(e) => setPrecoMaximo(e.target.value)} onKeyDown={(e) => e.key === "Enter" && buscar()} style={styles.input} />
          <div style={styles.buttonRow}><button onClick={buscar} style={styles.primaryButton}>Buscar</button><button onClick={limpar} style={styles.secondaryButton}>Limpar</button></div>
        </div>
        ...
        {resultados.map((e, i) => (
          <div key={i} style={styles.card}>
            ...
            <p><strong>Entrega:</strong> {formatarData(e.entrega)}</p>
            ...
            <div style={styles.tipologiaBox}>
              ...
              {e.tipologias && e.tipologias.length > 0 ? e.tipologias.map((t, idx) => (
                <div key={idx} style={styles.tipologiaLinha}>
                  {t.tipologia || "Tipologia"} • {t.areaMin ? `${String(t.areaMin).replace(".", ",")} m²` : "Área não informada"} • Varanda: {t.varanda || "Não informado"} • Vaga: {t.vaga || "Não informado"} • {t.precoTexto}
                </div>
...
const styles = {
  ...
  loginBox: { background: "#fff", padding: 24, borderRadius: 20, width: "100%", maxWidth: 360, textAlign: "center", boxShadow: "0 12px 30px rgba(0,0,0,0.08)" },
  loginTitle: { marginBottom: 16 },
  ...
  header: { background: "#fff", borderRadius: 22, padding: 20, display: "flex", gap: 14, alignItems: "center", marginBottom: 16, boxShadow: "0 12px 30px rgba(0,0,0,0.08)" },
  logo: { width: 54, height: 54, borderRadius: 14, background: "#0f3d91", color: "#fff", fontSize: 28, fontWeight: "bold", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  box: { background: "#fff", borderRadius: 18, padding: 18, textAlign: "center", marginBottom: 14, border: "1px solid #d8e4f8" },
  searchBox: { background: "#fff", borderRadius: 18, padding: 14, marginBottom: 14, boxShadow: "0 10px 24px rgba(0,0,0,0.05)" },
  ...
  error: { background: "#fee2e2", color: "#991b1b", borderRadius: 14, padding: 12, textAlign: "center", fontWeight: "bold", marginBottom: 14 },
  card: { background: "#fff", borderRadius: 20, padding: 18, marginBottom: 14, textAlign: "center", boxShadow: "0 12px 26px rgba(0,0,0,0.07)" },
  ...
  tipologiaBox: { marginTop: 14, borderTop: "1px solid #e5e7eb", paddingTop: 14 },
  tipologiaLinha: { background: "#f8fbff", border: "1px solid #d8e4f8", borderRadius: 12, padding: 10, marginTop: 8, fontSize: 14 },
  price: { color: "#059669", marginTop: 14 },
};
